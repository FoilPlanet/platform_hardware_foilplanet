...
OMXPluginBase *createOMXPlugin() {
    return new FpOMXPlugin;
}

FpOMXPlugin::FpOMXPlugin()
{
#if defined(USE_FOILPLANET_OMX)
...
OMX_ERRORTYPE FpOMXPlugin::AddCore(const char* coreName)
{
   bool isFPCore = false;
   ...
   if (libHandle != NULL) {
        FpOMXCore* core = (FpOMXCore*)calloc(1,sizeof(FpOMXCore));
		...
		if (isFPCore) {
            core->mInit = (FpOMXCore::InitFunc)dlsym(libHandle, "FpOMX_Init");
            core->mDeinit = (FpOMXCore::DeinitFunc)dlsym(libHandle, "FpOMX_DeInit");

            core->mComponentNameEnum =
            (FpOMXCore::ComponentNameEnumFunc)dlsym(libHandle, "FpOMX_ComponentNameEnum");

            core->mGetHandle = (FpOMXCore::GetHandleFunc)dlsym(libHandle, "FpOMX_GetHandle");
            core->mFreeHandle = (FpOMXCore::FreeHandleFunc)dlsym(libHandle, "FpOMX_FreeHandle");

            core->mGetRolesOfComponentHandle =
                (FpOMXCore::GetRolesOfComponentFunc)dlsym(
                        libHandle, "FpOMX_GetRolesOfComponent");

		} else {
            core->mInit = (FpOMXCore::InitFunc)dlsym(libHandle, "OMX_Init");
            core->mDeinit = (FpOMXCore::DeinitFunc)dlsym(libHandle, "OMX_Deinit");

            core->mComponentNameEnum =
            (FpOMXCore::ComponentNameEnumFunc)dlsym(libHandle, "OMX_ComponentNameEnum");

            core->mGetHandle = (FpOMXCore::GetHandleFunc)dlsym(libHandle, "OMX_GetHandle");
            core->mFreeHandle = (FpOMXCore::FreeHandleFunc)dlsym(libHandle, "OMX_FreeHandle");
...
OMX_ERRORTYPE FpOMXPlugin::makeComponentInstance(
        const char *name,
...
OMX_ERRORTYPE FpOMXPlugin::destroyComponentInstance(
        OMX_COMPONENTTYPE *component) {
    Mutex::Autolock autoLock(mMutex);
...
OMX_ERRORTYPE FpOMXPlugin::enumerateComponents(
        OMX_STRING name,
        size_t size,
        OMX_U32 index) {
...
OMX_ERRORTYPE FpOMXPlugin::getRolesOfComponent(
        const char *name,

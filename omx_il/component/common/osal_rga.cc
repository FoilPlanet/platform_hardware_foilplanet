...
#ifndef USE_DRM
    rga_ctx_t *ctx = NULL;
    ctx = mpp_malloc(rga_ctx_t, 1);
    memset(ctx, 0, sizeof(rga_ctx_t));
    ctx->rga_fd = -1;
    ctx->rga_fd = open("/dev/rga", O_RDWR, 0);
    if (ctx->rga_fd < 0) {
        mpp_err("rga open fail");
        return -1;
    }
    *rga_ctx = ctx;
    return 0;
...
#if defined(USE_RKVPU)

#ifndef USE_DRM
#define RGA_BUF_GEM_TYPE_DMA 0x80
OMX_S32 rga_copy(FoilplanetVideoPlane *plane, VPUMemLinear_t *vpumem, uint32_t Width, uint32_t Height, int format, int rga_fd)
{
    ...
    mpp_trace("rga start in");
    if (ioctl(rga_fd, RGA_BLIT_SYNC, &Rga_Request) != 0) {
        mpp_err("rga rga_copy fail");
        return -1;
    }
    mpp_trace("rga start out");
    return 0;
}

OMX_S32 rga_crop_scale(FoilplanetVideoPlane *plane,
                       VPUMemLinear_t *vpumem, OMX_VIDEO_PARAMS_EXTENDED *param_video,
                       OMX_U32 orgin_w, OMX_U32 orgin_h, int rga_fd)
{
    struct rga_req  Rga_Request;
    OMX_U32 new_width = 0, new_height = 0;
    memset(&Rga_Request, 0x0, sizeof(Rga_Request));
    ...
    if (ioctl(rga_fd, RGA_BLIT_SYNC, &Rga_Request) != 0) {
        mpp_err("rga_rgb2nv12 rga RGA_BLIT_SYNC fail");
        return -1;
    }
    mpp_trace("rga start out");
    return 0;
...
void rga_nv12_crop_scale(FoilplanetVideoPlane *plane,
                         VPUMemLinear_t *vpumem, OMX_VIDEO_PARAMS_EXTENDED *param_video,
                  ...
                  uint32_t Width, uint32_t Height, uint32_t dstWidth, uint32_t dstHeight,  void* rga_ctx)
{
    ...
    memset((void*)&src, 0, sizeof(rga_info_t));
    memset((void*)&dst, 0, sizeof(rga_info_t));
    mpp_trace(" plane->stride %d", plane->stride);
    ...
    src.fd = plane->fd;
    dst.fd = vpumem->phy_addr;
    mpp_trace("RgaBlit in src.fd = 0x%x, dst.fd = 0x%x", src.fd, dst.fd);
    if (RgaBlit(&src, &dst, NULL)) {
        mpp_err("RgaBlit fail");
    }
    mpp_trace("RgaBlit out");
#endif
}

void rga_nv122rgb( FoilplanetVideoPlane *planes, VPUMemLinear_t *vpumem, uint32_t Width, uint32_t Height, int dst_format, void* rga_ctx)
{
#ifndef USE_DRM
    rga_info_t src;
    rga_info_t dst;
...
void rga_nv12_copy(FoilplanetVideoPlane *plane, VPUMemLinear_t *vpumem, uint32_t Width, uint32_t Height, void* rga_ctx)
{
#ifndef USE_DRM
...
void rga_rgb_copy(FoilplanetVideoPlane *plane, VPUMemLinear_t *vpumem, uint32_t Width, uint32_t Height, void* rga_ctx)
{
#ifndef USE_DRM
